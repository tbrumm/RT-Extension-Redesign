...
| **Admin → Global → Login Banner** | WYSIWYG editor (CKEditor) for the maintenance/welcome banner — headline + German/English content, an Info/Warning type (blue/red), an optional expiry date, a live preview with a Light/Dark toggle that mirrors the OS-driven login page, and a floating "Unsaved changes" save bar that appears only once something changes |
...
The **system statistics** on the login page show: privileged/unprivileged users, groups, queues, tickets, transactions, assets, and articles — cached via `rt-redesign-stats-refresh`. The transaction figure is an approximate row estimate (from `information_schema`) rather than an exact `COUNT(*)`, so the refresh stays cheap on very large `Transactions` tables; each query in the refresh is also isolated so a single statement killed by a watchdog (e.g. pt-kill) degrades to a zero for that one figure instead of aborting the whole run. The login page and every admin dashboard show a "last update" timestamp so it is clear how current the figures are. The login page renders it in the RT server timezone (`$Timezone`, normally UTC); the admin dashboards render it in the logged-in user's own timezone preference. The login page is public, so until the refresh script has run it shows a neutral "statistics currently unavailable" note rather than computing the figures live on every visitor request (the run-the-script hint stays on the SuperUser-only admin dashboards).
